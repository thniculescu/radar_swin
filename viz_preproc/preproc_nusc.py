...
from utils import calc_ego_vel, get_points, split_time, split_ang_bins, filter_boxes, plot_proc_data, render_whole_sample
# %matplotlib inline
# %%
...
def preproc_scene(scene_id, nr_samps = 50, plots=False, max_range=50, nr_sweeps=6, ang_bins=360, save_output=False, min_radar_pts=1, min_lidar_pts=0):
    ...
    for nr_samps in range(min(nr_samps, nusc.scene[scene_id]['nbr_samples'] - 2)):
        ...
        if save_output:
            # os.makedirs(os.path.join(OUT_PREPROC_PATH, scene_name), exist_ok=True)
            # np.save(os.path.join(OUT_PREPROC_PATH, scene_name, 'radar' + str(nr_samps + 1) + '.npy'), sweep_frame)
            # np.save(os.path.join(OUT_PREPROC_PATH, scene_name, 'anns' + str(nr_samps + 1) + '.npy'), sweep_anns)
            # np.save(os.path.join(OUT_PREPROC_PATH, scene_name, 'ego_vels' + str(nr_samps + 1) + '.npy'), cur_velocity)
            # np.save(os.path.join(OUT_PREPROC_PATH, scene_name, 'heatmap' + str(nr_samps + 1) + '.npy'), box_heatmap)
    ...
    if save_output:
        np.save(os.path.join(OUT_PREPROC_PATH, scene_name + '.npy'), result, allow_pickle=True)
...
# start_time = timeit.default_timer()
# with Pool(15) as p:
#     p.map(partial(preproc_scene, plots=False, save_output=True), range(len(nusc.scene)))

# print('\n\nElapsed TOTAL time preproc dataset:', timeit.default_timer() - start_time)

# start_time = timeit.default_timer()
# np.save('nuscenes_mare2_preproc.npy', result, allow_pickle=True)
...
res = preproc_scene(0, nr_samps=2, plots=True, max_range=50, nr_sweeps=6, ang_bins=360, save_output=False, min_radar_pts=1, min_lidar_pts=0)
